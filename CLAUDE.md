...
- **The ledger colgroup is saturated.** At 1280px the table is 1215px and all 14 master columns sit within 0–4px of their content (`Ledger.tsx`, two arrays: `isBase` and proposals). There is no free width: giving a column more means taking it from a named neighbour or from control chrome. Before changing any percentage, measure — the widths carry comments saying what set them.
- **Intrinsic sizing lies about Hebrew in this font stack.** `width: max-content`, a detached clone and `canvas.measureText` all reported "משכנתא" at 38px while it painted at 38.3 and truncated to "משכנ…". To decide whether a label fits, append a positioned probe `<span>` with the same font-size/weight/letter-spacing/family and compare to the live box. `scrollWidth`/`clientWidth` on a shrunk flex item is not a reliable truncation signal either.
- Dialogs: `.lgr-ask*` in `theme.css` is the shared small-dialog skin (400px, question → receipt → two end-aligned buttons, primary last). `components/Ask.tsx` is the generic one, `DuplicateMasterModal` the arithmetic one. **No `window.confirm` on this surface** — a question the board asks belongs in the board's material.
