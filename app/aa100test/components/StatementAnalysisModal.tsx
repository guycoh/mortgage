...
function ShareBar({ parts }: { parts: { label: string; share: number; color: string; note?: string; amount: number }[] }) {
  ...
  return (
    <div>
      <div className="fin-split" role="img" aria-label="תמהיל">
        {shown.map((p) => (
      ...
      <ul className="mt-2 flex flex-wrap gap-x-4 gap-y-1.5">
        {shown.map((p) => (
          <li key={p.label} className="flex items-center gap-1.5 text-[11.5px]">
            ...
            <span className="fin-fig" style={{ color: "var(--ink-4)" }}>₪{fmt(p.amount)}</span>
    ...
    <tr key={t.uid} data-hl={lit?.uids.has(t.uid) || undefined} data-bad={(t.arrears ?? 0) > 0 || undefined}>
      <td>
        ...
        {t.balanceApportioned && (
          <span className="fin-chip !ms-1.5 !h-[17px] !px-1 !text-[9.5px]" title="הבנק אינו מפרט יתרה למסלול; חולקה לפי לוח הסילוקין">
            מחולק
          </span>
        )}
      </td>
      ...
      <td className="fin-fig" data-heat={t.rate !== null && t.rate >= 6 ? "hot" : t.rate !== null && t.rate >= 4.5 ? "warm" : undefined}>
        {rate(t.rate)}
      </td>
      <td className="fin-fig">{t.months ?? "—"}{t.monthsDerived && t.months ? "*" : ""}</td>
      <td className="fin-fig">{t.endDate || "—"}</td>
      <td className="fin-fig">{t.anchor ? `${t.anchor}${t.margin !== null ? ` ${t.margin > 0 ? "+" : ""}${t.margin}%` : ""}` : "—"}</td>
    ...
    >
      ...
      >
        <header className="fin-head">
          <h2 className="fin-title">ניתוח משכנתא</h2>
          <span className="fin-chip" style={{ color: "var(--ink-2)" }}>
            <Bank size={12} />
            {st.bankLabel}
          </span>
          {st.client.name && (
            <span className="fin-chip" style={{ color: "var(--ink-2)" }}>
              ...
              {st.client.name}
              {st.client.idNumber && <span className="fin-fig" style={{ color: "var(--ink-4)" }}>{st.client.idNumber}</span>}
            </span>
          )}
          {st.statementDate && (
        ...
        <div className="fin-kpis">
          ...
          <Kpi
            ...
            value={<span className="fin-fig fin-kpi-num">{rate(a.totals.rate)}</span>}
              ...
              <table className="fin-table fin-mini">
                ...
                <tbody>
                  {a.tracks.map((t) => (
                    <tr key={t.key}>
                      ...
                      <td className="fin-fig">{t.count}</td>
                      <td><Money value={t.balance} block={false} /></td>
                      <td className="fin-fig">{pct(t.share)}</td>
                ...
                <tfoot>
                  <tr>
                    ...
                    <td className="fin-fig font-bold">{a.live.length}</td>
                    <td><Money value={a.totals.balance} block={false} weight={800} /></td>
                    <td className="fin-fig font-bold">100%</td>
                    <td><Money value={a.totals.monthly} block={false} weight={800} /></td>
                    <td className="fin-fig font-bold">{rate(a.totals.rate)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
                  ...
                  <tbody>
                    {a.recycle.map((c) => (
                      <tr key={c.tranche.uid} data-hl={lit?.uids.has(c.tranche.uid) || undefined}>
                        ...
                        <td className="fin-fig" data-heat={(c.tranche.rate ?? 0) >= 6 ? "hot" : (c.tranche.rate ?? 0) >= 4.5 ? "warm" : undefined}>
                          {rate(c.tranche.rate)}
                        </td>
                        <td>{c.tranche.breakFee ? <Money value={c.tranche.breakFee} block={false} /> : "ללא"}</td>
                        <td className="fin-fig">
                    ...
                    {a.live
                      ...
                      .map((t) => (
                        <tr key={t.uid} data-hl={lit?.uids.has(t.uid) || undefined}>
                          ...
                          <td className="fin-fig">{t.baseIndex ?? "—"}</td>
                          <td className="fin-fig">{t.currentIndex ?? "—"}</td>
                        </tr>
                      ))}
                  </tbody>
                </table>
              ...
              <table className="fin-table fin-mini">
                ...
                <tbody>
                  <tr>
                    ...
                    <td className="fin-fig">{st.client.idNumber || "—"}</td>
                    <td className="fin-fig">{st.accountNumber || "—"}</td>
                    <td className="fin-fig">{st.statementDate || "—"}</td>
                    <td className="fin-fig">{st.loans.length}</td>
                  </tr>
                </tbody>
              </table>
            </div>
              ...
              <div className="mt-3 overflow-x-auto">
                ...
                <table className="fin-table fin-mini mt-1.5">
                  ...
                  <tbody>
                    {st.loans.map((l) => (
                      <tr key={l.loanNumber}>
                        <td className="fin-fig">{l.loanNumber}</td>
                        <td className="fin-fig">{l.tranches.length}</td>
