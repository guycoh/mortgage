      ...
      return {
        ...
        badges: (
          <span className="fin-chip">
            <span className="fin-fig font-bold">{subject.loans.length}</span> שורות בתמהיל
          </span>
        ),
        summary: {
    ...
    return {
      ...
      badges: (
        <>
          ...
          <span className="fin-chip">
            <span className="fin-dot" style={{ background: TRACK_HEX[subject.loan.path_id] }} />
            {PATH_LABEL[subject.loan.path_id]}
          </span>
          {res.isIndexed && <span className="fin-chip">צמוד מדד</span>}
        </>
      ),
      summary: {
  ...
  return createPortal(
    ...
    >
      <motion.div
        ...
        className="fin-card flex max-h-[86vh] w-full max-w-5xl flex-col overflow-hidden"
      ...
      >
        <header className="fin-head">
          ...
          <div className="ms-auto flex items-center gap-1.5">
            <button className="fin-btn fin-btn-sm" onClick={exportCsv}>
        ...
        <div className="fin-scroll flex-1 overflow-auto">
          <table className="fin-table">
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.month} className="fin-row">
                  <td className="fin-fig ps-3 text-[12px]" style={{ color: "var(--ink-3)" }}>
                    {r.month}
                  </td>
                  <td>
              ...
              {!rows.length && (
                <tr>
                  <td colSpan={6}>
                    <div className="fin-empty">אין נתונים — לשורות אין סכום או תקופה.</div>
                  </td>
                </tr>
              )}
            </tbody>
