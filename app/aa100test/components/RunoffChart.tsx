    ...
    chart.current.setOption(
      {
        ...
        tooltip: {
          ...
          axisPointer: {
            type: "line",
            lineStyle: { color: "#0e1524", width: 1, opacity: 0.35, type: [4, 3] },
          },
          backgroundColor: "#fff",
          borderColor: "#c6ccda",
          borderWidth: 1,
          padding: [8, 11],
          extraCssText:
            "border-radius:8px;box-shadow:0 4px 10px -2px rgba(14,21,36,.1),0 18px 38px -22px rgba(14,21,36,.45);direction:rtl;",
          ...
          formatter: (p: unknown) => {
            ...
            const head = `<div style="font-weight:700;font-size:12.5px">חודש ${m} <span style="color:#97a0b3;font-weight:500">· שנה ${yrs}</span></div>`;
            const lines = arr
              ...
              .map(
                (x) =>
                  `<div style="display:flex;align-items:center;gap:7px;margin-top:3px">
                     ...
                     <span style="color:#657189">${x.seriesName}</span>
                     <b dir="ltr" style="margin-inline-start:auto;font-family:'Archivo Variable',system-ui">₪${nis(Number(x.value))}</b>
                   </div>`
              )
              .join("");
            const foot = `<div style="display:flex;gap:10px;margin-top:6px;padding-top:5px;border-top:1px solid #e3e6ee">
                 <span style="color:#657189;font-weight:700">${mode === "balance" ? "יתרה" : "החזר"}</span>
                 <b dir="ltr" style="margin-inline-start:auto;font-family:'Archivo Variable',system-ui">₪${nis(total)}</b>
               </div>`;
        ...
        yAxis: {
          type: "value",
          axisLabel: { formatter: short, color: "#97a0b3", fontSize: 10, fontFamily: "Archivo Variable, system-ui" },
