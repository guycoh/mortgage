...
// הגנה קריטית ל-Vercel ולמובייל: הגדרת ה-Worker רק כאשר חלון הדפדפן קיים
...
export default function SignDocumentClient({ token }: { token: string }) {
  ...
  useEffect(() => {
    setIsMounted(true);
    // התיקון לקריסה בנייד: חישוב הרוחב מתבצע פעם אחת בלבד בעת טעינת העמוד!
    // הסרנו את ה-event listener שגרם לרינדור כפול בזמן גלילה
    if (typeof window !== "undefined") {
