...
export default function SpreadsheetPage() {
  ...
  const ExpandableTextarea = ({ name, value }: any) => {
    return (
      <div className="relative w-full h-5 group/textarea hover:z-[100] focus-within:z-[100]">
        <textarea
          ...
          className="absolute bottom-0 left-0 w-full min-w-0 h-5 py-0 px-1 text-xs font-medium text-right border rounded outline-none transition-all duration-200 resize-none overflow-hidden whitespace-pre-wrap leading-tight
            ...
            border-transparent bg-transparent text-gray-800 cursor-default group-hover/textarea:bg-white group-hover/textarea:border-gray-300"
        />
      </div>
    );
  }
                ...
                return (
                  <React.Fragment key={row.ui_id}>
                    ...
                    >
                      ...
                      <td className={`${tdClasses}`}><ExpandableTextarea name="additions" value={row.additions} /></td>
                    </tr>
                  </React.Fragment>
                );
              })}
