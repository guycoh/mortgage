...
const RunoffChart = dynamic(() => import("./RunoffChart"), {
  ssr: false,
  loading: () => <div className="lgr-skel m-3 h-[244px]" />,
});
...
function Composition({ loans }: { loans: ImportedLoan[] }) {
  ...
  if (!total) {
    return <div className="lgr-empty">הזינו סכומים או גררו דוח כדי לראות את הרכב התמהיל</div>;
  }

  return (
    <div className="px-3.5 pb-3.5 pt-3">
      {/* one bar, every track in proportion */}
      <div className="lgr-bar" dir="ltr">
        {byTrack.map((t, i) => {
          const pct = (t.amount / total) * 100;
          return (
            <motion.div
              key={t.id}
              className="lgr-bar-seg"
            ...
            >
              {pct > 8.5 && <span className="lgr-bar-pct">{Math.round(pct)}%</span>}
            </motion.div>
          );
        })}
      </div>
      ...
      <div className="mt-3 flex flex-col">
        {byTrack.map((t) => (
          ...
          >
            <span className="lgr-dot" style={{ background: TRACK_HEX[t.id] }} />
            <span className="flex items-baseline gap-1.5" style={{ color: "var(--lgr-2)" }}>
              {PATH_LABEL[t.id]}
              <span className="lgr-fig text-[10.5px]" style={{ color: "var(--lgr-4)" }}>
                ({t.count})
              </span>
            </span>
          ...
          return (
            ...
            >
              ...
              <div className="relative flex items-center gap-1.5 text-[11.5px] font-semibold" style={{ color: fam.text }}>
                ...
                {fam.plural}
                <span className="lgr-fig ms-auto opacity-80">{pct}%</span>
              </div>
              <Money value={amount} className="relative mt-1" size={16} weight={600} color="var(--ink)" style={{ textAlign: "start" }} />
              <Money value={monthly} per="ח׳" className="relative" size={11.5} color="var(--lgr-3)" style={{ textAlign: "start" }} />
            </div>
          );
        })}
      </div>
...
function Runoff({ loans, annualInflation }: { loans: ImportedLoan[]; annualInflation: number }) {
  ...
  if (!months.length) {
    return <div className="lgr-empty">צריך סכום ומספר חודשים כדי לצייר את מהלך התמהיל</div>;
  }
    ...
    <div>
      <div className="flex items-center gap-1.5 px-3.5 pt-3">
        {(
          ...
          <button key={k} className="lgr-tab !h-7 !text-[11.5px]" data-on={mode === k} onClick={() => setMode(k)}>
            {label}
          </button>
        ))}
        <span className="ms-auto flex items-center gap-1.5 text-[11px]" style={{ color: "var(--lgr-4)" }}>
          <span className="lgr-fig">{Math.round(maxMonth / 12)}</span> שנים
        </span>
      </div>
...
export default function Charts({
  ...
  return (
    <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_1.3fr]">
      <section className="lgr-card overflow-hidden">
        <header className="lgr-head">
          <ChartPieSlice size={14} weight="fill" style={{ color: "var(--lgr-3)" }} />
          <h2 className="lgr-title">הרכב התמהיל</h2>
          <span className="lgr-sub ms-auto">לפי מסלול</span>
        </header>
      ...
      <section className="lgr-card overflow-hidden">
        <header className="lgr-head">
          <TrendDown size={14} weight="fill" style={{ color: "var(--lgr-3)" }} />
          <h2 className="lgr-title">מהלך התמהיל</h2>
          <span className="lgr-sub ms-auto">מצטבר לפי מסלול</span>
        </header>
