  ...
  return (
    <section id={id} className="lgr-sec" data-hl={lit || undefined}>
      <header className="lgr-sec-head">
        ...
        {fold && (
          <button className="lgr-btn lgr-btn-sm ms-auto" onClick={() => setOpen((o) => !o)}>
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {go && <span className="lgr-flag-go">הצג במסמך<ArrowLeft size={11} weight="bold" /></span>}
        {flag.where?.length ? (
          <div className="mt-1 flex flex-wrap gap-1">
            {Array.from(new Set(flag.where)).slice(0, 6).map((w) => (
              <span key={w} className="lgr-chip !h-[19px] !px-1.5 !text-[10.5px]">
                {w}
              </span>
            ))}
          </div>
...
function ShareBar({
  ...
  return (
    <div>
      <div className="lgr-split" role="img" aria-label="התפלגות">
        {shown.map((p) => (
          <span
            key={p.label}
      ...
      <ul className="mt-2 flex flex-wrap gap-x-4 gap-y-1.5">
        {shown.map((p) => (
          <li key={p.label} className="flex items-center gap-1.5 text-[11.5px]">
            ...
            <span className="lgr-fig" style={{ color: "var(--lgr-4)" }}>
              ₪{fmt(p.amount)}
            </span>
            {p.note && (
              <span style={{ color: "var(--lgr-4)" }}>· {p.note}</span>
            )}
          </li>
        ))}
      </ul>
 ...
 * Bucket 6 is dark enough that its digit is set in white (see .lgr-heat).
 */
    ...
    <div className="overflow-x-auto">
      <table className="lgr-heat">
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.year}>
              <th className="lgr-fig">{r.year}</th>
              {r.months.map((v, i) => (
                <td
                  key={i}
                ...
                >
                  {v ? <span className="lgr-fig">{v}</span> : null}
                </td>
              ))}
            </tr>
          ))}
              ...
              <tr key={l.uid} data-bad={bad || undefined} data-hl={hl?.has(l.uid) || undefined}>
                {cols.map((c) => {
                  switch (c) {
                    case "bank":
                      return (
                        <td key={c} className="text-start">
                          ...
                          {bad && (
                            <span className="lgr-chip !ms-1.5 !h-[17px] !px-1 !text-[9.5px]" style={{ color: "var(--neg)", borderColor: "var(--neg-line)", background: "var(--neg-tint)" }}>
                    ...
                    case "type":
                      return (
                        <td key={c} className="text-start" style={{ color: "var(--lgr-3)" }}>
                          {l.type || "—"}
                        </td>
                      );
                    case "balance":
                    ...
                    case "rate":
                      return (
                        <td key={c} className="lgr-fig" data-heat={rateHeat(l.rate, l.category === "mortgage" ? "mortgage" : "loan") ?? undefined}>
                    ...
                    case "term":
                      return <td key={c} className="lgr-fig">{l.months ?? "—"}</td>;
                    case "end":
                      return <td key={c} className="lgr-fig">{l.endDate || "—"}</td>;
                    case "track":
                      return (
                        <td key={c} className="text-start" style={{ color: "var(--lgr-2)" }}>
                          {l.track || "—"}
                          {l.balloon && (
                            <span className="lgr-chip !ms-1.5 !h-[17px] !px-1 !text-[9.5px]" style={{ color: "var(--warn)", borderColor: "var(--warn-line, var(--line-2))" }}>
                              בלון
                            </span>
                          )}
                        </td>
                    ...
                    case "use":
                      return (
                        <td key={c} className="lgr-fig" data-heat={utilisationHeat(l.utilization === null ? null : l.utilization / 100) ?? undefined}>
                          {l.utilization === null ? "—" : `${l.utilization}%`}
                        </td>
                      );
                    case "status":
                      return (
                        <td key={c} className="text-start text-[11px]" style={{ color: "var(--lgr-4)" }}>
                          {l.status || "—"}
                        </td>
                      );
                  }
    ...
    >
      ...
      >
        ...
        <header className="lgr-head">
          <h2 className="lgr-title">ניתוח חיווי אשראי</h2>
          {a.clients.map((c, i) => (
            <span key={`${c.idNumber}-${i}`} className="lgr-chip" style={{ color: "var(--lgr-2)" }}>
              <IdentificationCard size={12} />
              {c.name || "ללא שם"}
              {c.idNumber && <span className="lgr-fig" style={{ color: "var(--lgr-4)" }}>{c.idNumber}</span>}
            </span>
          ))}
        ...
        <div className="lgr-kpis">
          ...
          <Kpi
            ...
            value={
              <span className="lgr-fig lgr-kpi-num">
              ...
              <table className="lgr-table lgr-mini">
                ...
                <tfoot>
                  <tr>
                    ...
                    <td className="lgr-fig font-bold">{own.length}</td>
                    <td><Money value={a.totals.balance} block={false} weight={800} /></td>
                    <td><Money value={a.totals.monthly} block={false} weight={800} /></td>
                    <td className="lgr-fig font-bold">{a.totals.rate === null ? "—" : `${a.totals.rate.toFixed(2)}%`}</td>
          ...
          {mortgages.length > 0 && (
            ...
            >
              ...
              {a.mortgage.collateralValue > 0 && (
                <p className="mt-2 text-[11.5px]" style={{ color: "var(--lgr-4)" }}>
                ...
                {[
                  ...
                  .map((x) => (
                    <div key={x.l}>
                      <Label>{x.l}</Label>
                      <div
                        className="lgr-fig text-[17px] font-extrabold"
                      ...
                      <tbody>
                        {a.inquiries.pending.map((q, i) => (
                          <tr key={`${q.user}-${i}`}>
                            ...
                            <td className="text-start" style={{ color: "var(--lgr-3)" }}>{q.transactionType || "—"}</td>
                            <td className="text-start" style={{ color: "var(--lgr-3)" }}>{q.purpose || "—"}</td>
                            <td className="lgr-fig">{q.amount || "—"}</td>
                            <td className="lgr-fig">{q.date || "—"}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              {a.inquiries.byPurpose.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {a.inquiries.byPurpose.map((p) => (
                    <span key={p.purpose} className="lgr-chip">
                      {p.purpose}
                      <span className="lgr-fig font-bold">{p.count}</span>
                    </span>
                  ))}
                </div>
              )}
                  ...
                  <table className="lgr-table lgr-mini mt-1.5">
                    ...
                    <tbody>
                      {a.legal.execution.map((c, i) => (
                        ...
                        >
                          <td className="text-start lgr-fig">{c["197-003"] || "—"}</td>
                          <td className="text-start" style={{ color: "var(--lgr-3)" }}>{c["197-004"] || "—"}</td>
                          <td className="lgr-fig">{c["197-006"] || "—"}</td>
                          <td className="lgr-fig">{c["197-007"] || "—"}</td>
                          <td className="lgr-fig">{c["197-009"] || "—"}</td>
                          <td className="lgr-fig">{c["197-013"] || "פתוח"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                    ...
                    <tbody>
                      {a.legal.insolvency.map((c, i) => (
                        <tr key={`${c["151-001"]}-${i}`} data-bad>
                          <td className="text-start lgr-fig">{c["151-001"] || "—"}</td>
                          <td className="text-start" style={{ color: "var(--lgr-3)" }}>{c["151-003"] || "—"}</td>
                          <td className="lgr-fig">{c["151-005"] || "—"}</td>
                          <td className="lgr-fig">{c["151-009"] || c["151-007"] || "—"}</td>
                ...
                <tbody>
                  {a.clients.map((c, i) => (
                    <tr key={`${c.idNumber}-${i}`}>
                      ...
                      <td className="text-start lgr-fig">{c.idNumber || "—"}</td>
                      <td className="text-start" style={{ color: "var(--lgr-3)" }}>{c.clientType || "—"}</td>
                      <td
                        className="text-start"
                        style={{ color: c.systemStatus && !/רגיל|תקין/.test(c.systemStatus) ? "var(--neg)" : "var(--lgr-3)" }}
                      >
                        {c.systemStatus || "—"}
                      </td>
                      <td className="lgr-fig">{c.collectionStart || "—"}</td>
                      <td className="lgr-fig">{c.reportDate || "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                ...
                <table className="lgr-table lgr-mini mt-1.5">
                  ...
                  <tbody>
                    {a.legal.adminActions.map((x, i) => (
                      <tr key={`${x.ref}-${i}`}>
                        <td className="lgr-fig" style={{ color: "var(--lgr-4)" }}>{x.ref || "—"}</td>
                        <td className="lgr-fig">{x.date || "—"}</td>
                        <td style={{ color: "var(--lgr-3)" }}>{x.type || "—"}</td>
                        <td style={{ color: /הסתיים|טופל/.test(x.status || "") ? "var(--lgr-3)" : "var(--warn)" }}>
                          {x.status || "—"}
                        </td>
                      </tr>
                    ))}
