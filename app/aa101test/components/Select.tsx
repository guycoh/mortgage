  ...
  return (
    <>
      <button
        ...
        type="button"
        className={`${
          variant === "input" ? "ink-input" : variant === "chip" ? "ink-fam-sel" : "ink-cell"
        } ink-sel-btn${className ? ` ${className}` : ""}`}
        style={style}
        data-text="true"
        data-open={open}
        data-empty={!current}
      ...
      >
        {current?.icon}
        {current?.dot && <span className="ink-dot" style={{ background: current.dot }} />}
        <span className="truncate">{current ? current.label : placeholder}</span>
        <CaretDown size={12} weight="bold" className="ink-sel-caret" />
      </button>
          ...
          <AnimatePresence>
            {open && pos && (
              <motion.div
                ...
                role="listbox"
                aria-label={ariaLabel}
                className="ink-pop ink-scroll ink-vars"
              ...
              >
                {options.map((o, i) => (
                  ...
                  >
                    {o.icon}
                    {o.dot && <span className="ink-dot" style={{ background: o.dot }} />}
                    <span className="truncate">{o.label}</span>
                    {o.note && <span className="ink-sel-note">{o.note}</span>}
                    {o.value === value && (
                      <Check size={13} weight="bold" className={o.note ? "ink-sel-check !ms-1.5" : "ink-sel-check"} />
                    )}
                  </button>
                ))}
              </motion.div>
