...
export default function LeadPicker({
  ...
  return (
    <>
      ...
      >
        <span className="ink-avatar" aria-hidden>
          {lead ? initial : <UserCircle size={16} weight="regular" />}
        </span>
        <span className="ink-picker-name">{name}</span>
        {lead && <span className="ink-picker-id">{lead.id}</span>}
        <CaretDown size={12} weight="bold" className="ink-picker-caret" />
      </motion.button>
            ...
            {open && pos && (
              ...
              >
                <div className="relative border-b p-2" style={{ borderColor: "var(--line)" }}>
                  ...
                  <input
                    autoFocus
                    className="ink-input w-full pe-8"
                ...
                <div className="ink-scroll max-h-[264px] overflow-auto p-1">
                  {busy && !rows.length && (
                  ...
                  {rows.map((l) => (
                    ...
                    >
                      ...
                      <span className="ink-sel-note">{l.id}</span>
                    </button>
                  ))}
                </div>
