...
const FAM_ICON = {
  mortgage: <Bank size={12} weight="fill" className="ink-fam-ico" />,
  loan: <HandCoins size={12} weight="fill" className="ink-fam-ico" />,
} as const;
  ...
  const addBtns = (size: "sm" | "md" = "sm") => (
    <div className="flex items-center gap-1.5">
      {ORDER.map((key) => (
        <Btn
          key={key}
          className={`ink-btn ink-btn-fam ${size === "sm" ? "ink-btn-sm" : ""}`}
          style={
            {
  ...
  return (
    ...
    <section className="ink-card">
      <header className="ink-head">
        <h2 className="ink-title">התחייבויות בתמהיל</h2>
        <span className="ink-count">{loans.length}</span>
          ...
          <table className="ink-table">
            ...
            <thead>
              <tr>
                ...
                <th>
                  <span className="ink-th-pair">
                    <span>עוגן %</span>
                    <span className="ink-th-pair-b">מרווח %</span>
                  </span>
                </th>
                <th>תדירות שינוי</th>
                <th>חודשים</th>
            ...
            <tbody>
              {groups.map((g) => {
                ...
                return (
                  <Fragment key={g.key}>
                    ...
                    <tr className="ink-groupbar" style={famVars}>
                      <td>
                        <div className="ink-groupbar-in">
                          <span className="ink-groupbar-title">
                            {FAM_ICON[g.key]}
                            {fam.plural}
                          </span>
                          <span className="ink-count">{g.rows.length}</span>
                        </div>
                      </td>
                      <td>
                        <Money value={g.amount} className="ink-groupbar-sum" />
                      </td>
                      <td colSpan={7} />
                      <td>
                        <Money value={g.monthly} className="ink-groupbar-sum" style={{ color: fam.text }} />
                      </td>
                      <td />
                    </tr>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td>
                            ...
                            />
                            <div className="ink-share mt-0.5" title={`${share.toFixed(1)}% מהתמהיל`}>
                          ...
                          {/* --- סכום --- */}
                          <td>
                            <div className="ink-well" data-dirty={dirty.has("amount") || undefined}>
                              <input
                                className="ink-cell ink-num-in font-bold"
                              ...
                              {(loan.source_bank || loan.is_guarantor || loan.is_shared) && (
                                <span className="ink-note truncate" style={{ maxWidth: "68%" }} title={loan.source_bank}>
                                  {loan.is_guarantor && (
                                    <span
                                      className="ink-tag me-1"
                          ...
                          {/* --- מסלול --- */}
                          <td>
                            <div className="ink-well" data-dirty={dirty.has("path_id") || undefined}>
                              <Select
                                value={loan.path_id}
                          ...
                          {/* --- לוח סילוקין --- */}
                          <td>
                            <div className="ink-well" data-dirty={dirty.has("amortization_schedule_id") || undefined}>
                              <Select
                          ...
                          <td>
                            <div className="ink-well" data-dirty={dirty.has("rate") || undefined}>
                              <input
                                className="ink-cell ink-num-in"
                                type="number"
                                step="0.01"
                          ...
                          <td>
                            ...
                            >
                              ...
                              />
                              {freqNote(loan) && <span className="ink-note">{freqNote(loan)}</span>}
                            </div>
                          </td>

                          {/* --- חודשים (synced with the end date) --- */}
                          <td>
                            <div className="ink-well" data-dirty={dirty.has("months") || undefined}>
                              <input
                                className="ink-cell ink-num-in"
                                type="number"
                                min={0}
                              ...
                              />
                              {months > 0 && <span className="ink-note">{(months / 12).toFixed(1)} שנ׳</span>}
                            </div>
                          </td>

                          {/* --- תאריך סיום --- */}
                          <td>
                            <div className="ink-well" data-dirty={dirty.has("loan_end_date") || undefined}>
                              <DateField
                          ...
                          <td>
                            <div className="ink-paycell">
                              {noTerm ? (
                                ...
                                <Money value={res.monthlyPayment} className="ink-pay" style={{ color: "var(--ink)" }} />
                              )}
                              {(res.isIndexed || stale) && (
                                <span className="ink-note ink-note-pay">
                                  {stale && <span className="ink-note-stale">תאריך עבר</span>}
              ...
              <tr className="ink-addrow">
                <td colSpan={11}>
                  <div className="ink-addrow-in">
                    {addBtns()}
                    <span className="ink-addrow-hint">הוספת שורה ריקה לתמהיל</span>
                  </div>
                </td>
              </tr>
            </tbody>
            ...
            <tfoot>
              <tr>
                <td>
                  <span className="ink-total-label">סה״כ</span>
                </td>
                <td>
                  <Money value={grand.amount} className="ink-total-fig" />
                </td>
                <td colSpan={7}>
                    ...
                    {groups
                      ...
                      .map((g) => (
                        ...
                        >
                          {FAM_ICON[g.key]}
                          {FAMILY[g.key].plural}
                          <span className="ink-fig" style={{ opacity: 0.75 }}>
                ...
                <td>
                  <Money value={grand.monthly} className="ink-total-fig" hot />
                </td>
                <td />
              </tr>
            </tfoot>
