...
function LenderRow({ r }: { r: ClientRow }) {
  ...
  return (
    <li className="ink-cs-row">
      <BankIcon source={r.bank} size={30} />
      <div className="min-w-0 flex-1">
        <div className="ink-cs-bank">{shortBank(r.bank) || r.bank}</div>
        <div className="ink-cs-meta">
          {bits.join(" · ")}
          {rate && (
            <>
              {bits.length ? " · " : ""}
              <span className={dear ? "ink-cs-warn" : undefined}>{rate}</span>
            </>
          )}
          {r.late && (
            <span className="ink-cs-warn">
              {" · בפיגור"}
...
function CardRow({ r }: { r: ClientRow }) {
  ...
  if (r.reported.limit && r.limit > 0) {
    meta.push(
      <span key="limit">
        מסגרת {ils(r.limit)} ₪
        {util !== null && (
          <span className={utilHeat === "hot" ? "ink-cs-warn" : utilHeat === "warm" ? "ink-cs-warm" : undefined}>
    ...
    meta.push(
      <span key="nolimit" className="ink-cs-warn">
  ...
  if (price) meta.push(<span key="rate" className={price.hot ? "ink-cs-warn" : undefined}>{price.text}</span>);
  if (r.late)
    meta.push(
      <span key="late" className="ink-cs-warn">
  ...
  if (enforced)
    meta.push(
      <span key="enf" className="ink-cs-warn">
        בטיפול ההוצאה לפועל
      </span>
    );

  return (
    <li className="ink-cs-row" data-alarm={r.late || enforced || undefined}>
      <BankIcon source={r.bank} size={30} />
      <div className="min-w-0 flex-1">
        <div className="ink-cs-bank">
          {shortBank(r.bank) || r.bank}
          <span className="ink-cs-kind">{r.type}</span>
        </div>
        <div className="ink-cs-meta ink-cs-meta-wrap">
          {meta.map((m, i) => (
            <span key={i} className="ink-cs-bit">
              {m}
            </span>
          ))}
        </div>
      ...
      <div className="ink-cs-amt">
        {r.reported.monthly ? (
          <>
            ...
            />
            {r.rolled > 0 && <div className="ink-cs-rolled">מזה {ils(r.rolled)} ₪ לא נפרעו</div>}
          </>
        ) : (
          <span className="ink-cs-none">לא דווח</span>
        )}
      </div>
    </li>
  );
...
function Section({ sec }: { sec: ClientSection }) {
  ...
  return (
    <section className="ink-cs-group">
      <h3 className="ink-cs-title" style={{ ["--fam" as string]: sec.accent }}>
        {sec.title}
      </h3>
      <div className="ink-cs-cols" aria-hidden>
        <span className="ink-cs-colspacer" />
        <span className="flex-1" />
        <span className="ink-cs-amt">יתרה</span>
        <span className="ink-cs-amt">לחודש</span>
      </div>
      <ul>
        {shown.map((r) =>
      ...
      {sec.rows.length > 1 && (
        <div className="ink-cs-sub">
          <span className="flex-1">סה״כ {sec.title}</span>
          <span className="ink-cs-amt">
...
export default function ClientSummaryModal({
  ...
  return createPortal(
    <div
      dir="rtl"
      className="ink-vars ink-printable fixed inset-0 z-[130] grid place-items-center p-4 backdrop-blur-[2px]"
    ...
    >
      <motion.div
        ...
        className="ink-card flex max-h-[92vh] w-full max-w-[760px] flex-col overflow-hidden"
      ...
      >
        <header className="ink-head">
          <div className="min-w-0">
            <h2 className="ink-display text-[19px] leading-tight">
        ...
        <div className="ink-cs min-h-0 flex-1 overflow-y-auto">
          {v.sections.map((sec) => (
          ...
          {v.worries.length > 0 && (
            <section className="ink-cs-worry">
              <div className="ink-cs-worry-head">
        ...
        <footer className="ink-cs-foot">
          <div>
            <div className="ink-cs-foot-cap">סך ההתחייבויות</div>
          ...
          <div className="text-end">
            <div className="ink-cs-foot-cap">יוצא מהחשבון כל חודש</div>
