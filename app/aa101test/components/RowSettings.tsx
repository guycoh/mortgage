...
export default function RowSettings({
  ...
  const field = (label: string, key: "grace_months", placeholder?: string) => (
    <label className="flex flex-col gap-1">
      <span className="ink-label">{label}</span>
      <div className="ink-well" data-dirty={dirty.has(key) || undefined}>
        <input
    ...
    <AnimatePresence>
      {pos && (
        <motion.div
          ref={ref}
          dir="rtl"
          className="ink-pop ink-sheet ink-vars"
        ...
        >
          <header className="ink-sheet-head">
            ...
            <h3 className="ink-title" style={{ fontSize: 12.5 }}>
              שדות נוספים
            </h3>
            <span
              className="ink-chip !h-[20px] !px-1.5 !text-[10.5px]"
            ...
            <button className="ink-act ms-auto !h-6 !w-6" onClick={onClose} aria-label="סגירה">
          ...
          <div className="ink-sheet-body">
            <label className="flex flex-col gap-1">
              <span className="ink-label">גרייס</span>
              <div className="ink-well" data-dirty={dirty.has("grace_type_id") || undefined}>
                <Select
          ...
          <footer className="ink-sheet-foot">
            {loan.source_track ? (
              <>
