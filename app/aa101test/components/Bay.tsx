...
function Mark() {
  return (
    <div className="ink-mark" aria-hidden>
...
export default function Bay({
      ...
      >
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 px-4 py-3">
          ...
          {reports.map((r, i) => (
            ...
            >
              <div className="flex items-center gap-2">
                <span className="ink-display text-[16px]">{r.clientName || "דוח ללא שם"}</span>
                {r.clientId && (
                  <span className="ink-chip !h-[21px] !px-2 !text-[10.5px]" style={{ color: "var(--ink-3)" }}>
                    <IdentificationCard size={11} />
                    <span className="ink-fig">{r.clientId}</span>
                  </span>
                )}
              </div>
              <div
            ...
            {(
              ...
              .map((f, i) => (
                ...
                >
                  {f.icon}
                  {FAMILY[f.key].plural}
                  <span className="ink-fig" style={{ opacity: 0.8 }}>
                    {f.n}
                  </span>
                </motion.span>
              ))}
          ...
          <div className="ms-auto flex items-center gap-1.5">
            ...
            <Btn className="ink-btn ink-btn-sm" onClick={pick} disabled={busy}>
              {busy ? (
            ...
            <Btn className="ink-act ink-tip" data-tip-side="start" data-tip="התחלה מחדש" onClick={onClear} aria-label="התחלה מחדש">
  ...
  return (
    ...
    >
      ...
      {drag && (
        <svg className="ink-ants" aria-hidden preserveAspectRatio="none">
          <rect x="0" y="0" width="100%" height="100%" rx="16" />
        </svg>
      )}
      {(drag || busy) && <span className="ink-beam" aria-hidden />}
      ...
      <div className="relative flex flex-wrap items-center gap-x-5 gap-y-3 px-4 py-3.5">
        ...
        <div className="min-w-[250px] flex-1">
          <div className="ink-display text-[17px]" style={{ color: error ? "var(--neg)" : "var(--ink)" }}>
            {headline}
          </div>
          <p
            className="mt-1 text-[12.5px] leading-snug"
            style={{ color: error ? "var(--neg)" : "var(--ink-3)" }}
          >
            {error || subline}
          </p>
          {/* what it takes, on the line it is relevant to */}
          <div className="ink-bay-foot">
            <span>PDF</span>
            <i />
        ...
        <span className="ink-btn ink-btn-primary flex-none !px-4" aria-hidden>
          {busy ? (
            <CircleNotch size={14} weight="bold" className="animate-spin" />
          ) : error ? (
            <ArrowClockwise size={14} weight="bold" />
        ...
        {error && (
          <Btn
            className="ink-act absolute end-2 top-2"
