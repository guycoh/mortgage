...
function ShareBar({ parts }: { parts: { label: string; share: number; color: string; note?: string; amount: number }[] }) {
  ...
  return (
    <div>
      <div className="ink-split" role="img" aria-label="תמהיל">
        {shown.map((p) => (
      ...
      <ul className="mt-2 flex flex-wrap gap-x-4 gap-y-1.5">
        {shown.map((p) => (
          <li key={p.label} className="flex items-center gap-1.5 text-[11.5px]">
            ...
            <span className="ink-fig" style={{ color: "var(--ink-4)" }}>₪{fmt(p.amount)}</span>
            {p.note && <span style={{ color: "var(--ink-4)" }}>· {p.note}</span>}
          </li>
        ))}
      </ul>
    </div>
    ...
    <tr key={t.uid} data-hl={lit?.uids.has(t.uid) || undefined} data-bad={(t.arrears ?? 0) > 0 || undefined}>
      <td>
        ...
        {t.balanceApportioned && (
          <span className="ink-chip !ms-1.5 !h-[17px] !px-1 !text-[9.5px]" title="הבנק אינו מפרט יתרה למסלול; חולקה לפי לוח הסילוקין">
            מחולק
          </span>
        )}
      </td>
      ...
      <td className="ink-fig" data-heat={t.rate !== null && t.rate >= 6 ? "hot" : t.rate !== null && t.rate >= 4.5 ? "warm" : undefined}>
        {rate(t.rate)}
      </td>
      <td className="ink-fig">{t.months ?? "—"}{t.monthsDerived && t.months ? "*" : ""}</td>
      <td className="ink-fig">{t.endDate || "—"}</td>
      <td className="ink-fig">{t.anchor ? `${t.anchor}${t.margin !== null ? ` ${t.margin > 0 ? "+" : ""}${t.margin}%` : ""}` : "—"}</td>
    ...
    >
      ...
      >
        <header className="ink-head">
          <h2 className="ink-title">ניתוח משכנתא</h2>
          <span className="ink-chip" style={{ color: "var(--ink-2)" }}>
            <Bank size={12} />
            {st.bankLabel}
          </span>
          {st.client.name && (
            <span className="ink-chip" style={{ color: "var(--ink-2)" }}>
              ...
              {st.client.name}
              {st.client.idNumber && <span className="ink-fig" style={{ color: "var(--ink-4)" }}>{st.client.idNumber}</span>}
            </span>
          )}
          {st.statementDate && (
            <span className="text-[11.5px]" style={{ color: "var(--ink-4)" }}>נכון ל-{st.statementDate}</span>
          )}
        ...
        <div className="ink-kpis">
          ...
          <Kpi
            ...
            value={<span className="ink-fig ink-kpi-num">{rate(a.totals.rate)}</span>}
        ...
        <div ref={bodyRef} className="ink-analysis min-h-0 flex-1 overflow-y-auto">
                ...
                <tbody>
                  {a.tracks.map((t) => (
                    <tr key={t.key}>
                      ...
                      <td className="ink-fig">{t.count}</td>
                      <td><Money value={t.balance} block={false} /></td>
                      <td className="ink-fig">{pct(t.share)}</td>
                      <td><Money value={t.monthly} block={false} /></td>
                      <td className="ink-fig">{rate(t.rate)}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr>
                    ...
                    <td className="ink-fig font-bold">{a.live.length}</td>
                    <td><Money value={a.totals.balance} block={false} weight={800} /></td>
                    <td className="ink-fig font-bold">100%</td>
                    <td><Money value={a.totals.monthly} block={false} weight={800} /></td>
                    <td className="ink-fig font-bold">{rate(a.totals.rate)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
          ...
          {a.recycle.length > 0 && (
            ...
            >
              <p className="mb-2.5 text-[12px] leading-relaxed" style={{ color: "var(--ink-3)" }}>
                  ...
                  <tbody>
                    {a.recycle.map((c) => (
                      <tr key={c.tranche.uid} data-hl={lit?.uids.has(c.tranche.uid) || undefined}>
                        ...
                        <td className="ink-fig" data-heat={(c.tranche.rate ?? 0) >= 6 ? "hot" : (c.tranche.rate ?? 0) >= 4.5 ? "warm" : undefined}>
                          {rate(c.tranche.rate)}
                        </td>
                        <td>{c.tranche.breakFee ? <Money value={c.tranche.breakFee} block={false} /> : "ללא"}</td>
                        <td className="ink-fig">
                  ...
                  <tbody>
                    {a.upcomingResets.map((t) => (
                      <tr key={t.uid} data-hl={lit?.uids.has(t.uid) || undefined}>
                        ...
                        <td><Money value={t.balance ?? 0} block={false} /></td>
                        <td className="ink-fig">{rate(t.rate)}</td>
                        <td className="ink-fig">{t.nextReset || "—"}</td>
                    ...
                    {a.live
                      ...
                      .map((t) => (
                        <tr key={t.uid} data-hl={lit?.uids.has(t.uid) || undefined}>
                          ...
                          <td className="ink-fig">{t.baseIndex ?? "—"}</td>
                          <td className="ink-fig">{t.currentIndex ?? "—"}</td>
                        </tr>
                      ))}
                  </tbody>
                </table>
              ...
              <table className="ink-table ink-mini">
                ...
                <tbody>
                  <tr>
                    ...
                    <td className="ink-fig">{st.client.idNumber || "—"}</td>
                    <td className="ink-fig">{st.accountNumber || "—"}</td>
                    <td className="ink-fig">{st.statementDate || "—"}</td>
                    <td className="ink-fig">{st.loans.length}</td>
                  </tr>
                </tbody>
              </table>
            </div>
              ...
              <div className="mt-3 overflow-x-auto">
                ...
                <table className="ink-table ink-mini mt-1.5">
                  ...
                  <tbody>
                    {st.loans.map((l) => (
                      <tr key={l.loanNumber}>
                        <td className="ink-fig">{l.loanNumber}</td>
                        <td className="ink-fig">{l.tranches.length}</td>
