...
const CompareChart = dynamic(() => import("./CompareChart"), {
  ssr: false,
  loading: () => <div className="ink-skel m-3 h-[268px]" />,
});
...
export default function Compare({
  ...
  if (!activeMix) return <div className="ink-empty">בחרו תמהיל להצגה.</div>;
  if (!activeMix.loans?.length)
    return <div className="ink-empty">אין נתונים להצגה עבור התמהיל הנוכחי — הזינו סכומים או גררו דוח.</div>;
  ...
  return (
    <div className="grid lg:grid-cols-2">
      {/* ---------------------------------------------------------- numbers */}
      <div className="ink-scroll overflow-x-auto">
        <table className="ink-table w-full table-fixed">
          <colgroup>
            <col style={{ width: "31%" }} />
            <col style={{ width: "23%" }} />
            <col style={{ width: "23%" }} />
          ...
          <tbody>
            {ROWS.map((row) => {
              ...
              return (
                <tr key={row.field} className="ink-row">
                  <td className="text-[12.5px] font-bold" style={{ color: "var(--ink-2)" }}>
                    {row.label}
                  </td>
                  <td>
                    {isRatio ? (
                      <span className="ink-money ink-money-block font-bold">{ratio(a)}</span>
                    ) : (
                  ...
                  <td>
                    {!other ? (
                      <span className="ink-calc" data-muted="true">
                        —
                      </span>
                    ) : isRatio ? (
                      <span className="ink-money ink-money-block" style={{ color: "var(--ink-3)" }}>
                        {ratio(b)}
                      </span>
                    ) : (
                  ...
                  <td>
                    {!other ? (
                      ...
                      <span className="ink-money ink-money-block font-bold" style={{ color }}>
                        {diff > 0 ? "+" : diff < 0 ? "−" : ""}
                        {ratio(Math.abs(diff))}
                      </span>
                    ) : (
      ...
      <div className="border-t lg:border-s lg:border-t-0" style={{ borderColor: "var(--line)" }}>
        {other && compareMix ? (
          <>
            ...
            >
              <span className="ink-display text-[15px]">
          ...
          <div className="ink-empty h-full">
