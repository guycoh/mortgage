...
export default function ScheduleModal({
      ...
      return {
        ...
        badges: (
          <span className="ink-chip">
            <span className="ink-fig font-bold">{subject.loans.length}</span> שורות בתמהיל
          </span>
        ),
        summary: {
    ...
    return {
      ...
      badges: (
        <>
          ...
          <span className="ink-chip">
            <span className="ink-dot" style={{ background: TRACK_HEX[subject.loan.path_id] }} />
            {PATH_LABEL[subject.loan.path_id]}
          </span>
          {res.isIndexed && <span className="ink-chip">צמוד מדד</span>}
        </>
      ),
      summary: {
  ...
  return createPortal(
    <div
      dir="rtl"
      className="ink-vars fixed inset-0 z-[120] grid place-items-center p-4 backdrop-blur-[2px]"
    ...
    >
      <motion.div
        ...
        className="ink-card flex max-h-[86vh] w-full max-w-5xl flex-col overflow-hidden"
        style={{ boxShadow: "var(--shadow-lift)" }}
        onClick={(e) => e.stopPropagation()}
      >
        <header className="ink-head">
          <h2 className="ink-title">{title}</h2>
          <div className="flex items-center gap-1.5">{badges}</div>
          <div className="ms-auto flex items-center gap-1.5">
            <button className="ink-btn ink-btn-sm" onClick={exportCsv}>
        ...
        <div className="grid grid-cols-2 border-b md:grid-cols-4" style={{ borderColor: "var(--line)" }}>
          {[
            ...
            >
              <div className="ink-label">{c.label}</div>
              <Money value={c.v} className="mt-0.5" size={17} weight={700} style={{ textAlign: "start" }} />
            </div>
          ))}
        </div>

        <div className="ink-scroll flex-1 overflow-auto">
          <table className="ink-table">
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.month} className="ink-row">
                  <td className="ink-fig ps-3 text-[12px]" style={{ color: "var(--ink-3)" }}>
                    {r.month}
                  </td>
                  <td>
              ...
              {!rows.length && (
                <tr>
                  <td colSpan={6}>
                    <div className="ink-empty">אין נתונים — לשורות אין סכום או תקופה.</div>
                  </td>
                </tr>
              )}
            </tbody>
