...
export default function DateField({
  ...
  useEffect(() => {
    ...
    const onPointer = (e: PointerEvent) => {
      ...
      if ((e.target as HTMLElement)?.closest?.(".ink-pop")) return;
      setOpen(false);
    };
    const onScroll = (e: Event) => {
      ...
      if (t instanceof HTMLElement && t.closest?.(".ink-pop")) return;
      setOpen(false);
    };
  ...
  return (
    <div ref={wrapRef} className="ink-well relative" data-dirty={undefined}>
      <input
        ref={inputRef}
        className="ink-cell pe-7"
      ...
      <button
        type="button"
        className="absolute inset-y-0 end-1.5 my-auto grid h-5 w-5 place-items-center rounded text-[var(--ink-4)] transition-colors hover:text-[var(--ink-2)]"
          ...
          <AnimatePresence>
            {open && pos && (
              <motion.div
                ref={popRef}
                dir="rtl"
                className="ink-pop ink-cal ink-vars"
              ...
              >
                <div className="ink-cal-head">
                  {/* RTL: the "previous month" affordance points right */}
                  <button className="ink-cal-nav" onClick={() => shiftMonth(-1)} aria-label="חודש קודם" type="button">
                  ...
                  <button className="ink-cal-nav" onClick={() => shiftMonth(1)} aria-label="חודש הבא" type="button">
                    <CaretLeft size={13} weight="bold" />
                  </button>
                </div>

                <div className="ink-cal-grid">
                  {HE_DOW.map((d) => (
                    <div key={d} className="ink-cal-dow">
                      {d}
                    </div>
                  ))}
                  {cells.map(({ date, out }) => (
                ...
                <div className="ink-cal-foot">
                  <button className="ink-btn ink-btn-sm" type="button" onClick={() => pick(today)}>
                    היום
                  </button>
                  <button
                  ...
                  <span className="ink-cal-hint">{hint ?? "אפשר גם להקליד dd/mm/yyyy"}</span>
                </div>
              </motion.div>
            )}
          </AnimatePresence>,
