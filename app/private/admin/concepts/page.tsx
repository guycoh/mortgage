...
const handleSaveEdit = (updatedItem: DataItem) => {
  setData((prev) =>
    prev ? prev.map((item) => (item.id === updatedItem.id ? updatedItem : item)) : null
  )

  // אופציונלי: שליחת PUT ל־API לעדכון בשרת
  fetch(`/api/concepts/${updatedItem.id}`, {
    method: "PUT",
    headers: {
    ...
    alert("שגיאה בעדכון הנתון בשרת: " + err.message)
  })
}
...
const confirmDelete = async () => {
  ...
  try {
    const res = await fetch(`/api/concepts/${conceptToDelete}`, {
      method: 'DELETE',
    })

    if (!res.ok) throw new Error('Delete failed')
    setData((prev) => prev!.filter((item) => item.id !== conceptToDelete))
  
    setToast({ message: 'המושג נמחק בהצלחה', type: 'success' })
  } catch (error) {
