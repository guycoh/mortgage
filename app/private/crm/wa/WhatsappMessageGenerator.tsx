...
setMessage(msg);
  ...
  return (
  <div className="max-w-7xl mx-auto p-6 bg-white shadow-md rounded-xl" dir="rtl">
        ...
        <div className="w-full max-w-md mx-auto">
          ...
          <div className="relative">
            <textarea
              ...
              readOnly
              rows={3}
              className="w-full p-3 pr-20 rounded-md border border-gray-300 bg-gray-50 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-orange-400"
              defaultValue={`💼 זה הכרטיס ביקור שלי:\nhttps://morg-orcin.vercel.app/card`}
              onClick={(e) => (e.target as HTMLTextAreaElement).select()}
            />
            <button
              onClick={() => {
                const textarea = document.getElementById("bizcard") as HTMLTextAreaElement;
                navigator.clipboard.writeText(textarea.value);
              }}
              className="absolute top-2 right-2 bg-orange-500 hover:bg-orange-600 text-white px-3 py-1 rounded-md text-sm transition-all"
            >
              העתק
            </button>
          </div>
